...
|--------|-----------|-----------|
| `x = 1` | присваивание переменной | `let x = 1` / `const x = 1` |
| `UPPER_CASE` | константа по соглашению (не язык) | `const` — защита от перезаписи |
| `int` | целые числа бесконечной точности | `Number` (всегда 64-bit float) |
| `10 / 2 == 5.0` | всегда возвращает float | `10 / 2 === 5` (number) |
| `10 // 3 == 3` | целочисленное деление | нет прямого аналога |
| `2 ** 10` | оператор степени | `Math.pow(2, 10)` / `2**10` (ES2016) |
| `None` | единственное пустое значение | `null` + `undefined` |
| `bool([]) == False` | пустые коллекции falsy | `Boolean([]) === true` |
| `bool({}) == False` | пустой dict falsy | `Boolean({}) === true` |
| `isinstance(True, int) == True` | bool наследует int | `typeof true === 'boolean'` |
| `a, b = 1, 2` | множественное присваивание | `const [a, b] = [1, 2]` |
| `a, b = b, a` | swap без temp-переменной | `[a, b] = [b, a]` |
