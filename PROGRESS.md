...
**Текущая тема:** Не начато
**Следующий шаг:** Начать с Phase 1, Topic 1 — Variables, types, type hints
...
- [ ] 1.1 — Переменные и типы: `int`, `float`, `bool`, `None`, `complex` — как они устроены, динамическая типизация
- [ ] 1.2 — Числа подробно: целочисленная арифметика без переполнения, `//`, `%`, `**`, `Decimal`, `round()`
...
- [ ] 3.3 — GIL (Global Interpreter Lock): что это, почему, последствия (+ GIL в 3.13)
