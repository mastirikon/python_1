...
**Текущая тема:** 1.15 — `bytes` и `bytearray`
**Следующий шаг:** Phase 1, Topic 1.15 — `bytes` и `bytearray`: что это, encode/decode, зачем нужны
...
- [x] 1.3 — `bool` как подкласс `int` — питоновский сюрприз: `True + True == 2`
...
- [x] 1.9 — Управляющие конструкции: `if/elif/else`, тернарный оператор, `match/case` (3.10+)
- [x] 1.10 — Циклы: `for`, `while`, `break`, `continue`, `else` у циклов (редкая фича)
- [x] 1.11 — Списки: создание, индексация, срезы, `append`, `extend`, `insert`, `pop`, `sort`
- [x] 1.12 — Кортежи: иммутабельность, именованные кортежи (`namedtuple`, `typing.NamedTuple`)
- [x] 1.13 — Словари: создание, методы, `dict.get()`, `.items()`, `.keys()`, `.values()`, merge (`|`)
- [x] 1.14 — Множества: `set`, `frozenset`, операции (`&`, `|`, `-`, `^`), когда использовать
...
| 2026-03-28 | 1.3 — bool как подкласс int | Пройдено: truthy/falsy, bool наследует int, коллизии в dict, enumerate. Решены 3 задачи. |
...
| 2026-04-11 | 1.9 — Управляющие конструкции | Пройдено: if/elif/else, chained comparison, тернарный оператор, match/case с деструктуризацией, guard, OR-паттерны. Решены 3 задачи. |
| 2026-04-15 | 1.10 — Циклы | Пройдено: for, while, break, continue, else у циклов, enumerate, zip, range. Решены 3 задачи. |
| 2026-04-15 | 1.11 — Списки | Пройдено: создание, индексация, срезы, методы, sort vs sorted, ловушка умножения вложенных списков. Решены 3 задачи. |
| 2026-04-19 | 1.12 — Кортежи | Пройдено: иммутабельность, распаковка, namedtuple, NamedTuple, zip-паттерн. Решены 3 задачи. |
| 2026-04-23 | 1.13 — Словари | Пройдено: создание, методы, get, items/keys/values, merge, dict comprehension. Решены 3 задачи. |
| 2026-04-23 | 1.14 — Множества | Пройдено: set, frozenset, операции (|, &, -, ^), update, дедупликация, type hints. Решены 3 задачи. |
